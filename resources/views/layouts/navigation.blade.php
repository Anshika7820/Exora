<nav x-data="{ open: false }" class="bg-black border-b border-white/5 sticky top-0 z-[100] backdrop-blur-md bg-black/80">
        ...
        <div class="flex justify-between h-20">
            <div class="flex">
                ...
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('dashboard') }}" class="flex items-center space-x-3">
                        ...
                        <span class="text-2xl font-black tracking-tighter gradient-text">Exora</span>
                    </a>
                </div>
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-4 py-2 border border-white/10 text-sm leading-4 font-black rounded-full text-gray-300 bg-white/5 hover:bg-white/10 hover:text-white focus:outline-none transition ease-in-out duration-150">
                    ...
                    <x-slot name="content">
                        <div class="px-4 py-2 border-b border-white/5 bg-gray-900/50">
                            <div class="text-xs font-black text-gray-500 uppercase tracking-widest">{{ auth()->user()->role === 'creator' ? 'Hoster' : 'Participater' }} Account</div>
                        </div>
                        ...
                        <x-dropdown-link href="#" class="text-gray-300 hover:bg-white/5">
                            {{ __('Account Settings') }}
                        </x-dropdown-link>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden bg-gray-900 border-t border-white/5">
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')" class="text-gray-300">
                {{ __('Dashboard') }}
            </x-responsive-nav-link>
            <x-responsive-nav-link :href="route('hall')" :active="request()->routeIs('hall')" class="text-gray-300">
                {{ __('Hall') }}
            </x-responsive-nav-link>
            <x-responsive-nav-link :href="route('auditorium')" :active="request()->routeIs('auditorium')" class="text-gray-300">
                {{ __('Auditorium') }}
            </x-responsive-nav-link>
        ...
        <div class="pt-4 pb-1 border-t border-white/5">
            ...
            <div class="mt-3 space-y-1">
                ...
                <form method="POST" action="{{ route('logout') }}">
                    ...
                    <x-responsive-nav-link :href="route('logout')" class="text-red-400"
                            onclick="event.preventDefault();
                                        this.closest('form').submit();">
                        {{ __('Log Out') }}
                    </x-responsive-nav-link>
